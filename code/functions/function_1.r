...
run_simulation_1 <- function(m_n1, iterations = 10, m_M0 = 24, nt = 75) {
  nn <- 150
  n <- 200
  mse_list <- vector("list", iterations)
  ...
  for (iteration in 1:iterations) {
    ...
    mse_iter <- parallel::mclapply(1:m_M0, function(j) {
      nbasis <- 2 * j + 2
      evaluate_model(nbasis, x, y, true, m_n1, xt, nn, nt)
    }, mc.cores = detectCores() - 1)
    mse_list[[iteration]] <- unlist(mse_iter)
  }
  mse <- Reduce("+", mse_list) / iterations
  mse
}
